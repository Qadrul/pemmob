import 'dart:async';
import 'dart:convert';
import 'package:anime_verse/services/auth/firestore_service.dart';
...
class AppStateProvider extends ChangeNotifier {
  ...
  void _subscribeToFavorites(String userId) {
    ...
    _favoritesSubscription = _firestoreService.getFavouriteStream(userId).listen(
      (favorites) {
...
void _unsubscribeFromFavorites() {
  ...
  Future <void> addFavorite(Anime anime) async {
    ...
    if (user != null) {
      await _firestoreService.addFavourite(user.uid, anime);
    }
  }
  ...
  Future <void> removeFavorite(int malId) async {
    final user = FirebaseAuth.instance.currentUser;
    if (user != null) {
      await _firestoreService.removeFavourite(user.uid, malId);
    }
  }
